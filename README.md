...
* 用state属性来表示资源的状态（分独占模式和共享模式），子类需要定义如何维护这个状态，控制如何获取和释放锁
  * getState获取state状态
  * setState设置state状态
  * compareAndSetState cas设置state状态
  * 独占模式是一个只有一个线程能够访问资源，而共享模式可以允许多个线程访问资源
...
在此模式下，即使他被打断，仍会驻留在AQS队列中，等获得锁后方能继续运行（是继续运行！只是打断标记被设置为true）
...
提供一个`数据容器类`内部分别使用锁保护数据的read()方法，写锁保护数据的write()方法
...
读写锁用的是同一个Sync同步器，因此等待队列和state也是同一个
...
循环栅栏，用来进行线程协作，等待线程满足某个计数，构造时设置【计数个数】，每个线程执行到某个需要“同步”的时刻调用await()方法进行等待，当等待的线程数满足【技术个数】时，继续执行

`CyclicBarrier` 每次执行完 `await()`，它的 `count` 属性会减 1，执行一次循环后，`count` 会置为 0，下一次怎么执行呢？

**答案是：** 当 `CyclicBarrier` 的计数器达到 0 后，它会自动重置为初始值（即 `parties` 的数量），并且你可以继续使用它进行下一次同步。

### 线程安全集合类概述

重点介绍java.util.concurrent.*下的线程安全集合类，可以发现它们拥有规律，里面包含三类关键词：
...
> 遍历时如果发生了修改，对于非安全容器来讲，使用fail-fast机制也就是让遍历立刻失败，抛出异常不再继续遍历
...
* JDK8虽然讲扩容算法做了调整，不再将元素加入链表头（而是保持与扩容前一样的顺序），但仍不意味着能够在多线程环境下能够安全扩容，还可能会出现其它问题（如扩容丢失数据）
...
主要列举LinkedBlockinngQueue和ArrayBlockingQueue的性能比较

* LInked支持有界，Array强制有界
* Linked实现是链表，Array实现是数组
* Linked是懒惰的，Array需要提前初始化Node数组
* Linked每次入队会生成新的Node，Array的Node是提前创建好的
* Linked两把锁，Array一把锁
